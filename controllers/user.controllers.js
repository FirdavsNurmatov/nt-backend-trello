...
export async function getAllData(req, res, next) {
  try {
    const getData = await pool.query("SELECT id, name, email, createdat, updatedat FROM users");
    res.status(200).send({
      status: "Success",
      data: getData.rows,
    });
...
export async function searchData(req, res, next) {
  try {
    ...
    const searchData = await pool.query(
      "SELECT id, name, email, createdat, updatedat FROM users WHERE name ILIKE '%$1%'",
      [q]
    );
    res.status(200).send({
      status: "Success",
...
export async function getById(req, res, next) {
  try {
    ...
    const data = await pool.query("SELECT id, name, email, createdat, updatedat FROM users WHERE id=$1", [id]);
    res.status(200).send({
      status: "Success",
      data: data.rows[0],
    });
